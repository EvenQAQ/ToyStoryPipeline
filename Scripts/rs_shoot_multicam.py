...
class DeviceManager:
    ...
    def enable_device(self, device_serial, enable_ir_emitter):
        ...
        self._enabled_devices[device_serial] = (
            Device(pipeline, pipeline_profile))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str,
                        default="test", help="store filename")
    parser.add_argument("--width", type=int,
                        default=640, help="image width")
    ...
    try:
        ...
        while True:
            ...
            # print(frames)
            for (serial, frameset) in frames.items():
                if not serial in writers:
                    writers[serial] = cv2.VideoWriter(
                        output_file + '-' + serial + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
                print(serial)
                print(frameset[rs.stream.color])
                cv2.namedWindow('RealSense' + serial, cv2.WINDOW_AUTOSIZE)
                depth_image[serial] = np.asanyarray(
                    frameset[rs.stream.depth].get_data())
                color_image[serial] = np.asanyarray(
                    frameset[rs.stream.color].get_data())
                depth_colormap[serial] = cv2.applyColorMap(cv2.convertScaleAbs(
                    depth_image[serial], alpha=0.03), cv2.COLORMAP_JET)
                images = np.hstack(
                    (color_image[serial], depth_colormap[serial]))
                cv2.imshow('RealSense' + serial, color_image[serial])
                writers[serial].write(color_image[serial])
            key = cv2.waitKey(1)
            if key == 32:
                num_image += 1
                print('shoot' + str(num_image))
                for serial in frames:
                    cv2.imwrite(output_file + '-color-' + serial + '-' + str(num_image) +
                                '.png', color_image[serial])
                    cv2.imwrite(output_file + '-depth-' + serial + '-' + str(num_image) +
                                '.png', depth_image[serial])
                    cv2.imwrite(output_file + '-map-' + serial +
                                '-' + str(num_image) + '.png', depth_colormap[serial])
