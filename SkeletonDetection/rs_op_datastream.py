...
tf_config = tf.ConfigProto() #对session进行参数配置
tf_config.allow_soft_placement = True #如果你指定的设备不存在，允许TF自动分配设备
tf_config.gpu_options.per_process_gpu_memory_fraction = 0.9  #分配百分之七十的显存给程序使用，避免内存溢出，可以自己调整
tf_config.gpu_options.allow_growth = True  #按需分配显存，这个比较重要
...
openpose_dir = 'D:/DevTools/openpose/build/examples/tutorial_api_python'
...
if __name__ == '__main__':
    ...
    cv2.namedWindow('Skeleton Display', cv2.WINDOW_AUTOSIZE)
    ...
    try:
        ...
        dir_path = 'D:/DevTools/openpose/build/examples/tutorial_api_python'
        ...
        for i in range(0, len(args_openpose[1])):
            ...
            if i != len(args_openpose[1])-1:
                next_item = args_openpose[1][i+1]
            else:
                next_item = "1"
        ...
        while True:
            ...
            boxes_quli = []
            for box in boxes:
                box_scaled = [box[1]*cap_params['im_width'], box[0]*cap_params['im_height'],
                              box[3]*cap_params['im_width'], box[2]*cap_params['im_height']]
            ...
            if len(boxes) >= 2:
                boxes_hand = [[max(boxes[0][1]*cap_params['im_width'], 0), max(boxes[0][0]*cap_params['im_height'], 0), min(boxes[0][3]*cap_params['im_width'], cap_params['im_width']-1), min(boxes[0][2]*cap_params['im_height'], cap_params['im_height']-1)], [
                    max(boxes[1][1]*cap_params['im_width'], 0), max(boxes[1][0]*cap_params['im_height'], 0), min(boxes[1][3]*cap_params['im_width'], cap_params['im_width']-1), min(boxes[1][2]*cap_params['im_height'], cap_params['im_height']-1)]]
                box_left_final = boxes_hand[0]
                imageToProcess = cv2.rectangle(imageToProcess, (int(box_left_final[0]), int(
                    box_left_final[1])), (int(box_left_final[2]), int(box_left_final[3])), (153, 153, 153), -1)
            ...
            if pose_keypoints.size > 1:
                pose3ds = threeFromDepth(
                    depth_filtered, depth_scale, clip_long, clip_short, image_max, depth_intrin, pose_keypoints[0])
            elapsed_time = (datetime.datetime.now() -
            的
                if (args.display > 0):
                    if (args.fps > 0):
                        detector_utils.draw_fps_on_image("FPS : " + str(int(fps)),image_out)
                        cv2.imshow('Skeleton Display', image_out)
                        key = cv2.waitKey(15)
